    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # taetae latents = torch.Size([8, 4, 64, 64])
                ...
                # taetae Resize controlnet_image to match noisy_latents 
                controlnet_image = controlnet_image.unsqueeze(2)  # Add a height dimension
                controlnet_image = torch.nn.functional.interpolate(
                    controlnet_image, size=(64, 64), mode="bilinear", align_corners=False
                )
                import torch.nn as nn 

                # Reduce channels to match noisy_latents
                channel_reducer = nn.Conv2d(in_channels=128, out_channels=8, kernel_size=1).to(controlnet_image.device)
                controlnet_image = channel_reducer(controlnet_image)    


                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_latents,
                    # noisy_latents = torch.Size([8, 4, 64, 64])
                    timesteps,
